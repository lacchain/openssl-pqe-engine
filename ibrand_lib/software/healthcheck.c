...
{
    bool rc;
    // JG hack: double entropy = inmHealthCheckEstimateEntropyPerBit();

    // JG hack: rc = (inmTotalBits >= INM_MIN_DATA) &&
    // JG hack:      (entropy*INM_ACCURACY >= inmExpectedEntropyPerBit) &&
    // JG hack:      (entropy/INM_ACCURACY <= inmExpectedEntropyPerBit);

    //rc = (inmTotalBits >= INM_MIN_DATA);
    rc = 1;
    if (!rc)
    {
        printf("Waiting for more data (inmTotalBits(%lu) < INM_MIN_DATA(%u))\n", inmTotalBits, INM_MIN_DATA);
        // JG hack: printf("(entropy*INM_ACCURACY >= inmExpectedEntropyPerBit)=%d\n", (entropy*INM_ACCURACY >= inmExpectedEntropyPerBit));  // <=== Failing
...
bool inmEntropyOnTarget(uint32_t entropy, uint32_t numBits)
{
    ...
    rc = expectedEntropy < entropy*INM_ACCURACY;
    rc = 1;
    return rc;
}
