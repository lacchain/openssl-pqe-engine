...
static OQS_STATUS kem_test_correctness(const char *method_name)
{
    ...
    // JG: It seems that the input buffer, shared_secret_e, has not been initialised with anything specific!!!
    ...
    // JG: It seems that the original shared secret, shared_secret_e, was never explicitly initialised with anything specific!!!
