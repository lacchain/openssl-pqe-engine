...
int main(int argc, char **argv) {
    ...
    if (opts.serial == NULL) {
        opts.serial = getenv("IBRAND_SERIAL");
    }

    if (!opts.debug) {
        char *envDbg = getenv("IBRAND_DEBUG");
        opts.debug = (envDbg != NULL
                      && !strcmp("true", envDbg));
    }

    if (!multiplierAssigned) {
        char *envMultiplier = getenv("IBRAND_MULTIPLIER");
    ...
    if (opts.listDevices) {
        ibrand_devlist_node_t* devlist = listUSBDevices(&context.message);
        if (devlist == NULL) {
            fprintf(stderr, "Error: %s\n", context.message);
            return 1;
        }
        ibrand_devlist_node_t* curdev;
        int id = 0;
        for (curdev = devlist; curdev != NULL; id++) {
            ...
            // cleanup:
            ibrand_devlist_node_t* olddev = curdev;
            curdev = curdev->next;
            free(olddev);
        }
        return 0;
    ...
    if (opts.devRandom) {
#ifdef LINUX
        inmWriteEntropyStart(BUFLEN / 8u, opts.debug); // TODO: create method in libibrand.h for this?
    ...
    // initialize USB device, health check and Keccak state (see libibrand)
    if (!initIBRand(&context, opts.serial, !opts.raw, opts.debug)) {
        fprintf(stderr, "Error: %s\n", context.message);
        return 1; // ERROR
    }
