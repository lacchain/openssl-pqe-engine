...
uint32_t readData_Protected(struct ibrand_context *context, uint8_t *result, bool raw, uint32_t outputMultiplier)
{
    ...
    else
    {
...
#if (IB_SOURCE_OF_RANDOMNESS == RANDSRC_FILE)
        {
            ...
            size_t fileSize;
            int  bytesRead;
            char *mallocedResult = GetValueFromConfigFile("IBRAND_CONF","IBRandDatafilePath");
            if (mallocedResult)
            {
                szIBDatafilename = mallocedResult;
            }
            ...
            rewind(fIBDatafile);
            if (fileSize < sizeof(inBuf))
            {
                context->message = "ERROR: Insufficient data in IBDatafile";
                context->errorFlag = true;
                fclose(fIBDatafile);
                printf("%s - (requested=%lu, actual=%lu)\n", context->message, sizeof(inBuf), fileSize);
                return 0;
            }

            // Read the data
            bytesRead = fread(inBuf, sizeof(char), sizeof(inBuf), fIBDatafile);
            if (bytesRead != sizeof(inBuf))
            {
                ...
                fclose(fIBDatafile);
                printf("c");
                printf("%s - (requested=%lu, bytesRead=%d)\n", context->message, sizeof(inBuf), bytesRead );
                return 0;
            }
