...
void *my_malloc(unsigned int size, char purpose)
{
    ...
    {
        printf("ERROR: Too many mallocs. Please increase MLOG_COUNT\n");
        return NULL;
    }
    ptr = malloc(size);
    if (!ptr)
    {
        printf("ERROR: malloc failure size=%u purpose=%c\n", size, purpose);
        return NULL;
    }
    memset(ptr, 0, size);
...
void my_free(void *ptr)
{
    ...
    if (found == -1)
    {
        printf("ERROR: Attempted free of unknown ptr %p\n", ptr);
        return;
    }
    if (mlog[found].s != 'A')
    {
        printf("ERROR: Attempted free of previously freed ptr %p\n", ptr);
        return;
    }
    if (mlog[found].p == UNALLOCATED_PTR || mlog[found].p == 0)
    {
        printf("ERROR: Attempted free of invalid ptr %p\n", ptr);
        return;
    }
...
void *my_realloc(void *oldptr, unsigned int newsize, char newpurpose)
{
    ...
    if (found == -1)
    {
        printf("ERROR: Attempted realloc of unknown ptr %p\n", oldptr);
        return NULL;
    }
    if (mlog[found].s != 'A')
    {
        printf("ERROR: Attempted realloc of previously freed ptr %p\n", oldptr);
        return NULL;
    }
    ...
    if (!newptr)
    {
        printf("ERROR: realloc failure size=%u purpose=%c\n", newsize, newpurpose);
        return NULL;
    }
...
void my_mlogdump(char *title)
{
    ...
    for (ii=0;ii<MLOG_COUNT;ii++)
    {
        ...
        printf("    mlog[%3d]: p=%p n=%4d s=%c y=%c\n",ii, mlog[ii].p, mlog[ii].n, mlog[ii].s, mlog[ii].y);
        countActive++;
    }
    printf("    mlog stats:\n");
    printf("        Active mallocs              : %u/%u\n",countActive,MLOG_COUNT);
    printf("        Allocated/Freed/Other       : %u/%u/%u\n",countStatus[0],countStatus[1],countStatus[2]);
    printf("        MLOG_PURPOSE_ROOTOBJECT     : %u\n", countPurpose[0]);
    printf("        MLOG_PURPOSE_ROOTPAIR       : %u\n", countPurpose[1]);
    printf("        MLOG_PURPOSE_NEWKEY         : %u\n", countPurpose[2]);
    printf("        MLOG_PURPOSE_NEWCHILDOBJECT : %u\n", countPurpose[3]);
    printf("        MLOG_PURPOSE_NEWCHILDSTRING : %u\n", countPurpose[4]);
    printf("        MLOG_PURPOSE_NEWSTRINGVALUE : %u\n", countPurpose[5]);
    printf("        MLOG_PURPOSE_EXTENDPAIRS    : %u\n", countPurpose[6]);
    printf("        Other purpose               : %u\n", countPurpose[7]);
}
...
void my_freeJSONFromMemory(JSONObject *obj)
{
    ...
    if (obj)
    {
        dbg_stmnt_hv1(printf("DEBUG: Found root object (ptr=%p)\r\n", obj));
        if (obj->pairs)
        {
            dbg_stmnt_hv1(printf("DEBUG: Found array of %d pairs (ptr=%p)\r\n", obj->count, obj->pairs));
            for (i = 0; i < obj->count; i++)
            {
                if (obj->pairs[i].key != NULL)
                {
                    dbg_stmnt_hv1(printf("DEBUG: Found key on pair %d (ptr=%p)\r\n", i, obj->pairs[i].key));
                    dbg_stmnt_hv2(printf("DEBUG: (heap:free) key size=(%2d+1) ptr=%p key=\"%s\"\n",i,obj->pairs[i].key,obj->pairs[i].key));
                    my_delete(obj->pairs[i].key);
                }
                if (obj->pairs[i].value != NULL)
                {
                    dbg_stmnt_hv1(printf("DEBUG: Found value on pair %d (ptr=%p)\r\n", i, obj->pairs[i].value));
                    switch (obj->pairs[i].type)
                    {
                        case JSON_STRING:
                        {
                            dbg_stmnt_hv1(printf("DEBUG: Found string on value on pair %d (ptr=%p)\r\n", i, obj->pairs[i].value->stringValue));
                            dbg_stmnt_hv2(printf("DEBUG: (heap:free) strvalue size=(%2d+1) ptr=%p val=\"%s\"\n",i,obj->pairs[i].value->stringValue,obj->pairs[i].value->stringValue));
                            my_delete(obj->pairs[i].value->stringValue);
                            break;
                        }
                        case JSON_OBJECT:
                        {
                            dbg_stmnt_hv1(printf("DEBUG: Found childobj on value on pair %d (recursing with ptr=%p)\r\n", i, obj->pairs[i].value->jsonObject));
                            my_freeJSONFromMemory(obj->pairs[i].value->jsonObject);
                            dbg_stmnt_hv1(printf("DEBUG: Back from recursion of childobj on value on pair %d (recursing)\r\n", i));
                            break;
                        }
                    }
                    dbg_stmnt_hv1(printf("DEBUG: We can now free the value struct on pair %d (ptr=%p)\r\n", i, obj->pairs[i].value));
                    dbg_stmnt_hv2(printf("DEBUG: (heap:free) val size=(%2d+1) ptr=%p\n",0,obj->pairs[i].value));
                    my_delete(obj->pairs[i].value);
                }
            }
            // Added by JG
            dbg_stmnt_hv1(printf("DEBUG: We can now free the array of pairs (ptr=%p)\r\n", obj->pairs));
            dbg_stmnt_hv2(printf("DEBUG: (heap:free) rootpairs array ptr=%p\n",obj));
            my_delete(obj->pairs);
        }
        dbg_stmnt_hv1(printf("DEBUG: We can now free the root object (ptr=%p)\r\n", obj));
        dbg_stmnt_hv2(printf("DEBUG: (heap:free) rootobj ptr=%p\n",obj));
        my_delete(obj);
    }
#ifdef DEBUG_MALLOC_USAGE
...
static JSONObject * _parseJSON(const char * str, int *pOffset)
{
    ...
    dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) rootobj size=(%2d) ptr=%p\n",sizeof(JSONObject),obj));
    ...
    dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) rootpair size=(%2d) ptr=%p\n",sizeof(JSONPair),obj->pairs));
    ...
    while (*str != '\0')
    {
        ...
        if (*str == JSON_STARTOFOBJECT)
        {
            ...
            {
                printf("ERROR: Error parsing JSON string - Unexpected '%c' after '%c' (or SOT)\r\n", *str, prevToken);
                while(true);
                return NULL;
            }
        ...
        else if (*str == JSON_STARTOFKEYSTRING)
        {
            ...
            {
                printf("ERROR: Error parsing JSON string - Unexpected '%c' after '%c'\r\n", *str, prevToken);
                while(true);
                return NULL;
            }
            ...
            tempPtr.key[i] = '\0';
            dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) newkey size=(%2d+1) ptr=%p key=\"%s\"\n",i,tempPtr.key,tempPtr.key));
            dbg_stmnt_hv3(printf("DEBUG: newkey key=\"%s\"\n",tempPtr.key));
            ...
            if (*str == JSON_STARTOFOBJECT)
            {
                ...
                dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) childobj size=(%2d) ptr=%p\n",sizeof(JSONValue),tempPtr.value));
            ...
            else if (*str == JSON_STARTOFVALUESTRING)
            {
                ...
                dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) childstr size=(%2d) ptr=%p\n",sizeof(JSONValue),tempPtr.value));
                ...
                dbg_stmnt_hv2(printf("DEBUG: (heap:malloc) strval size=(%2d+1) ptr=%p val=\"%s\"\n",i,tempPtr.value->stringValue,tempPtr.value->stringValue));
                dbg_stmnt_hv3(printf("DEBUG: val=\"%s\"\n",tempPtr.value->stringValue));
        ...
        else if (*str == JSON_PAIRDELIMITER)
        {
            ...
            dbg_stmnt_hv2(printf("DEBUG: (heap:free) val ptr=%p\n",obj->pairs));
            obj->pairs = my_renewWithSize(obj->pairs, JSONPair, obj->count, MLOG_PURPOSE_EXTENDPAIRS);
            dbg_stmnt_hv2(printf("DEBUG: (heap:realloc) extendpairs size=(%2d+1) ptr=%p\n",(sizeof(JSONPair)*obj->count),obj->pairs));
    ...
    {
        printf("ERROR: Error parsing JSON string - Unexpected EOT after '%c'\r\n", prevToken);
        while(true);
        return NULL;
    }
